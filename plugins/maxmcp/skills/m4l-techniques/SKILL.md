...
```mermaid
flowchart TD
  ltd["live.thisdevice"] -- "bang<br/>(デバイス + LOM 完全初期化後)" --> tbb["t b b"]
  tbb -- "out 1 (fires first)" --> s_lom["s ---init_lom"]
  s_lom -. "LOM チェーン起動<br/>(live.path bang 等)" .-> r_lom["receivers"]
  tbb -- "out 0 (fires second)" --> delay["delay 100"]
  delay --> s_params["s ---init_params"]
  s_params -. "パラメータ復元の二段目" .-> r_params["receivers"]
```
