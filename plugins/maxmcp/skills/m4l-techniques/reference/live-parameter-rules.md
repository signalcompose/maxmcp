...
```
✅ 正解:
sel 1 outlet 0 (Learn ON) → t b b
                              ├→ zl.reg property selected_parameter → observer inlet 0
...
```
依存チェーン:
  param_min (pack inlet 1, cold) → pack → _parameter_range → range_min / range_max
  param_max (pack inlet 0, hot) ↗                              ↓
                                            range_max (pak, 中間発火) → pak → _parameter_range → scaled_display
                                            range_min (pak, 最終発火) ↗
```
