...
**検証手順**:
1. `get_objects_in_patch({mode: "layout"})` で全オブジェクトの `varname`・`position`・`size` を取得 (text/maxclass を省略してトークン削減)
...
| ツール | フェーズ | 用途 |
|---|---|---|
| `get_objects_in_patch` | 分析 | 全オブジェクト情報取得 (Phase 1 はフル、Phase 8 検証は `mode: "layout"` でトークン削減) |
| `get_patchlines` | 分析 | 全接続情報取得 (Phase 8 検証は `mode: "geometry"` でトークン削減、既存パッチ解析時は `mode: "connections"`) |
